...
// The signIn/signOut helpers run only in the browser: they fetch a CSRF token
// and submit a hidden POST form (Auth.js v5 rejects a plain GET to its action
// endpoints). The test environment is Node, so we stub the minimal `fetch` and
// `document` surface they touch and assert on the form that gets built and
// submitted — the behavior, not a hard-coded URL string.
